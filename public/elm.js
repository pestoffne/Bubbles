				...
				[
					...
					shape: A2(
						...
						A2(
							...
							$ianmackenzie$elm_units$Length$cssPixels,
							_Utils_Tuple2(100, 256)),
						$ianmackenzie$elm_units$Length$cssPixels(100)),
					velocity: A2(
						$ianmackenzie$elm_geometry$Vector2d$withLength,
						$ianmackenzie$elm_units$Length$cssPixels(2.5),
					...
					shape: A2(
						...
						A2(
							$ianmackenzie$elm_geometry$Point2d$fromTuple,
							$ianmackenzie$elm_units$Length$cssPixels,
							_Utils_Tuple2(300, 256)),
						$ianmackenzie$elm_units$Length$cssPixels(30)),
					velocity: A2(
						$ianmackenzie$elm_geometry$Vector2d$withLength,
						$ianmackenzie$elm_units$Length$cssPixels(5),
...
var $author$project$Main$burst = F2(
	function (model, mouseEvent) {
		var isTouched = F2(
			function (touchPos, bubble) {
				return A2(
					$ianmackenzie$elm_geometry$Circle2d$contains,
					A2($ianmackenzie$elm_geometry$Point2d$fromTuple, $ianmackenzie$elm_units$Length$cssPixels, touchPos),
					bubble.shape);
			});
		var touchedBubbles = A2(
			$elm$core$List$filter,
			isTouched(mouseEvent.offsetPos),
			model.bubbles);
		var isNotTouched = F2(
			function (touchPos, bubble) {
				return !A2(isTouched, touchPos, bubble);
			});
		var untouchedBubbles = A2(
			$elm$core$List$filter,
		...
		return _Utils_update(
			model,
			{
				...
				scores: model.scores + $elm$core$List$length(touchedBubbles)
			});
	});
...
var $author$project$Main$moveBubbles = function (bubbles) {
	var moveBubble = function (bubble) {
		return _Utils_update(
			bubble,
			{
				shape: A2($ianmackenzie$elm_geometry$Circle2d$translateBy, bubble.velocity, bubble.shape)
			});
	};
	return A2($elm$core$List$map, moveBubble, bubbles);
};
		...
		if (message.$ === 'Tick') {
			return _Utils_Tuple2(
				_Utils_update(
					model,
					{
						bubbles: $author$project$Main$moveBubbles(model.bubbles),
						tick: model.tick + 1
					}),
				$elm$core$Platform$Cmd$none);
		} else {
			...
			return _Utils_Tuple2(
				A2($author$project$Main$burst, model, mouseEvent),
				$elm$core$Platform$Cmd$none);
		}
	});
...
var $author$project$Main$height = 512;
...
var $author$project$Main$renderBubbles = function (bubbles) {
	var shapeBubble = function (bubble) {
		return A2(
			$joakin$elm_canvas$Canvas$circle,
			A2(
				$ianmackenzie$elm_geometry$Point2d$toTuple,
				$ianmackenzie$elm_units$Length$inCssPixels,
				$ianmackenzie$elm_geometry$Circle2d$centerPoint(bubble.shape)),
			$ianmackenzie$elm_units$Length$inCssPixels(
				$ianmackenzie$elm_geometry$Circle2d$radius(bubble.shape)));
	};
	return A2(
		$joakin$elm_canvas$Canvas$shapes,
		_List_fromArray(
		...
		A2($elm$core$List$map, shapeBubble, bubbles));
};
...
var $author$project$Main$renderScores = function (model) {
	return A3(
		...
		_Utils_Tuple2(4, 6),
		'Scores = ' + ($elm$core$String$fromInt(model.scores) + (', Tick = ' + $elm$core$String$fromInt(model.tick))));
};
			...
			[
				...
				_List_fromArray(
					[
						...
						$mpizenberg$elm_pointer_events$Html$Events$Extra$Mouse$onDown($author$project$Main$StartAt)
					]),
				_List_fromArray(
					[
						$author$project$Main$clearCanvas,
						$author$project$Main$renderBubbles(model.bubbles),
						$author$project$Main$renderScores(model)
					]))
			]));
};
